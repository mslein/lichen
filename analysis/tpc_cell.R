...
cell_npp <- read_csv("analysis/tidy data/cell_npp_final.csv")  
cell_r <- read_csv("analysis/tidy data/cell_r_final.csv") 
cell_gpp <- read_csv("analysis/tidy data/cell_gpp_final.csv") 

cell_npp_unweighted <- cell_npp %>% filter(is.na(sd_molgCmin))
cell_npp_weighted <- cell_npp %>% filter(!is.na(sd_molgCmin))

cell_r_unweighted <- cell_r %>% filter(is.na(sd_molgCmin))
cell_r_weighted <- cell_r %>% filter(!is.na(sd_molgCmin))

cell_gpp_unweighted <- cell_gpp %>% filter(is.na(sd_molgCmin))
cell_gpp_weighted <- cell_gpp %>% filter(!is.na(sd_molgCmin)) 
...
#unweighted
results_npp_unweighted <- cell_npp_unweighted %>%
...
cell_npp_unweighted_plot <- ggplot(plot_data_npp_unweighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
...
ggsave("figures/qc/cell_npp_unweighted_plot.png", cell_npp_unweighted_plot, height=15, width=20)



#weighted
results_npp_weighted <- cell_npp_weighted %>%
  group_by(as.factor(tpc_grp)) %>%
  group_split() %>%
...
cell_npp_weighted_plot <- ggplot(plot_data_npp_weighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
  #scale_color_manual(values=c( "#a559aa", "#59a89c", "#f0c571", "#ae282c","#082a54" ))+
...
ggsave("figures/qc/cell_npp_weighted_plot.png", cell_npp_weighted_plot, height=15, width=20)


#########R#######

#unweighted
results_r_unweighted <- cell_r_unweighted %>%
  group_by(as.factor(tpc_grp)) %>%
  group_split() %>%
  lapply(fit_and_predict)
...
cell_r_unweighted_plot <- ggplot(plot_data_r_unweighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
  #scale_color_manual(values=c( "#a559aa", "#59a89c", "#f0c571", "#ae282c","#082a54" ))+
...
ggsave("figures/qc/cell_r_unweighted_plot.png", cell_r_unweighted_plot, height=15, width=20)


#weighted
results_r_weighted <- cell_r_weighted %>%
  group_by(as.factor(tpc_grp)) %>%
  group_split() %>%
  lapply(fit_and_predict_w)
...
cell_r_weighted_plot <- ggplot(plot_data_r_weighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
  #scale_color_manual(values=c( "#a559aa", "#59a89c", "#f0c571", "#ae282c","#082a54" ))+
...
ggsave("figures/qc/cell_r_weighted_plot.png", cell_r_weighted_plot, height=15, width=20)


#########GPP#######

#unweighted
results_gpp_unweighted <- cell_gpp_unweighted %>%
  group_by(as.factor(tpc_grp)) %>%
  group_split() %>%
  lapply(fit_and_predict)
...
cell_gpp_unweighted_plot <- ggplot(plot_data_gpp_unweighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
  #scale_color_manual(values=c( "#a559aa", "#59a89c", "#f0c571", "#ae282c","#082a54" ))+
...
ggsave("figures/qc/cell_gpp_unweighted_plot.png", cell_gpp_unweighted_plot, height=15, width=20)



#weighted
results_gpp_weighted <- cell_gpp_weighted %>%
  group_by(as.factor(tpc_grp)) %>%
  group_split() %>%
  lapply(fit_and_predict_w)
...
cell_gpp_weighted_plot <- ggplot(plot_data_gpp_weighted, aes(x = Temperature, y = rate, color=tpc_grp)) +
  #scale_color_manual(values=c( "#a559aa", "#59a89c", "#f0c571", "#ae282c","#082a54" ))+
...
ggsave("figures/qc/cell_gpp_weighted_plot.png", cell_gpp_weighted_plot, height=15, width=20)
...
write_csv(tpc_params, "analysis/tidy data/cell_tpc_Eas.csv")
